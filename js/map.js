...
function get_started() {
	if (!mlb_lightbox.is_visible()) {
		mlb_lightbox.show({ "iframe": "getstarted.html", "title": "Loading...", "content": "Loading...", width: '800', height: '600' });
	}
}
function close_mlb() {
	$('.mlb-close').click();
...
function call_windfarm_api(geojson, callback) {
	$.ajax({
		type: "POST",
...
function to_geo_json_object(feature) {
	return JSON.parse(to_geo_json(feature));
}
...
function power_text_style(feature, resolution) {
	...
	if (resolution < 250) {
		if (feature.get("wind_power_total") === undefined) {
			return 'Calculating';
		}
		return `
...
function load_layers_applic() {
	...
	var layers_list = [
		...
		},
		{
			...
			url: "https://scomp1184.wur.nl/geoserver/WMRwms/wms",
